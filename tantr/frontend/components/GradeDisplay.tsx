...
export default function GradeDisplay({
  ...
  const vibe = rubric?.vibe;
  const vibeScore = typeof vibe?.score === 'number' ? vibe.score : null;
    ...
    <Card className="animate-slide-up border-cyan-500/10">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {vibeScore != null && (
            <span
              className="rounded-full border border-fuchsia-500/25 bg-fuchsia-500/10 px-2.5 py-0.5 font-mono text-xs text-fuchsia-300"
              title={vibe?.explanation || 'Heuristic estimate only'}
            >
      ...
      {(explanations.final || explanations.difficulty || vibe?.explanation) && (
        <div className="mt-6 space-y-2 border-t border-violet-500/10 pt-4">
          ...
          {vibe?.explanation && (
            <p className="text-sm text-slate-500">{vibe.explanation}</p>
          )}
          {vibe?.signals && vibe.signals.length > 0 && (
            <ul className="list-inside list-disc space-y-1 font-mono text-xs text-slate-600">
              {vibe.signals.slice(0, 5).map((s, i) => (
                <li key={i}>{s.detail || s.id}</li>
              ))}
            </ul>
          )}
